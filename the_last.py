...
seta_cima = [1, 2, 3, 4]       
seta_baixo = [7]    
seta_esquerda = [5, 6, 8, 9]   
seta_direita = [10, 11, 12, 13]

# Lista para armazenar a sequência de direções
sequencia_direcoes = []

# Carregar as imagens das setas
imagem_seta_cima = pygame.image.load('cimaa.png')
imagem_seta_baixo = pygame.image.load('baixoo.png')
imagem_seta_esquerda = pygame.image.load('esquerdaa.png')
imagem_seta_direita = pygame.image.load('direitaa.png')
...
imagem_bob_cima = pygame.transform.scale(imagem_bob_cima, (65, 65))

imagem_bob_baixo = pygame.image.load('bob_baixo.png')
imagem_bob_baixo = pygame.transform.scale(imagem_bob_baixo, (65, 65))

imagem_bob_esquerda = pygame.image.load('bob_esq.png')
imagem_bob_esquerda = pygame.transform.scale(imagem_bob_esquerda, (65, 65))

imagem_bob_direita = pygame.image.load('bob_dir.png')
imagem_bob_direita = pygame.transform.scale(imagem_bob_direita, (65, 65))
...
imagem_X = pygame.image.load('xzao.png')
imagem_X = pygame.transform.scale(imagem_X, (70, 70))

imagem_coringa = pygame.image.load('palhaco.png')
imagem_coringa = pygame.transform.scale(imagem_coringa, (65, 65))

# imagem atual como a de cima
...
def main(nivel):
    ...
    global sequencia_direcoes, imagem_bob, imagem_X
    ...
    coringas = []
    while len(coringas) < qtde_coringas:  # Definir a quantidade de coringas
        linha = random.randint(0, linhas - 1)
        coluna = random.randint(0, colunas - 1)
        if (linha, coluna) not in coringas and linha !=0 and linha !=1:  # Evitar duplicação e sem gerar nas duas últimas linhas
            coringas.append((linha, coluna))

    # pos_x e pos_y estão definidos --> agr calcular bob_x e bob_y
    ...
    while running:
        ...
        # 0.5 segundo para mostrar o tabuleiro depois de mostrar a sequência completa
        time.sleep(0.5)
        ...
        # tabuleiro é exibido, agora jogador pode tentar acertar a sequência
        ...
        while jogando and teclas_pressionadas < fase:
            for event in pygame.event.get():
                ...
                if event.type == KEYDOWN and var == 1:
                    if event.key == K_UP:
                        movimento = seta_cima
                        imagem_bob = imagem_bob_cima
                    ...
                    if teclas_usuario != sequencia_direcoes[:len(teclas_usuario)]:
                        if teclas_usuario[-1] == seta_cima:
                            screen.blit(imagem_X, (bob_x, bob_y - altura))
                            pygame.display.flip()
                        elif teclas_usuario[-1] == seta_baixo:
                            screen.blit(imagem_X, (bob_x, bob_y + altura))
                            pygame.display.flip()
                        elif teclas_usuario[-1] == seta_esquerda:
                            screen.blit(imagem_X, (bob_x - largura, bob_y))
                            pygame.display.flip()
                        elif teclas_usuario[-1] == seta_direita:
                            screen.blit(imagem_X, (bob_x + largura, bob_y))
                            pygame.display.flip()
                            
                        time.sleep(2)
                        # sequência do jogador incorreta --> o jogo termina
                        tela_derrota(fase)
                        running = False
                        jogando = False
                        break
                    ...
                    if teclas_pressionadas < fase:
                        if teclas_pressionadas < fase:
                            # Dentro do loop principal de eventos, logo após capturar o evento e verificar o movimento
                            if movimento == seta_cima:
                                if bob_y-20 > pos_y:  # Movimento permitido dentro da matriz (gamb - 20)
                                    bob_y -= altura
                                ...
                                if bob_x-20 > pos_x:  # Limite esquerdo da matriz (gamb - 20)
                                    bob_x -= largura
                ...
                # Verificar se Bob colidiu com o Coringa e mover imediatamente
                for linha, coluna in coringas[:]:
                    coringa_x = pos_x + coluna * (largura + margem)
                    coringa_y = pos_y + linha * (altura + margem)

                    # Verificar colisão
                    if abs(bob_x - coringa_x) < largura and abs(bob_y - coringa_y) < altura:
                        if qtde_coringas == 1 or qtde_coringas == 2:
                            bob_y -= altura  # Move Bob para cima imediatamente
                        elif qtde_coringas == 3:
                            bob_y -= altura*2  # Move Bob para cima imediatamente
                            
                        # Remove o Coringa da lista
                        coringas.clear()
                        break  # Só deve mover quando colidir com um Coringa
            ...
            for linha, coluna in coringas:
                x = pos_x + coluna * (largura + margem)+4
                y = pos_y + linha * (altura + margem)+4
                screen.blit(imagem_coringa, (x, y))


            
            # Exibir a imagem de Bob
            screen.blit(imagem_bob, (bob_x + 4, bob_y + 2))
            pygame.display.flip()  # Atualizar a tela
            clock.tick(60)
